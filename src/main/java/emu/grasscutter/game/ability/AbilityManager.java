...
public final class AbilityManager extends BasePlayerManager {
  private static final org.slf4j.Logger log =
      org.slf4j.LoggerFactory.getLogger(AbilityManager.class);
  ...
  public void executeMixin(Ability ability, AbilityMixinData mixinData, ByteString abilityData) {
    ...
    if (handler == mixinHandlers.get(AbilityMixinData.Type.PhlogistonCostMixin)) {
      ...
      if (avatar.getAvatarId() == 10000106
          || avatar.getAvatarId() == 10000107
          || avatar.getAvatarId() == 10000105
          || avatar.getAvatarId() == 10000103
          || avatar.getAvatarId() == 10000100) {
    ...
    if (handler == mixinHandlers.get(AbilityMixinData.Type.SwitchHealToHPDebtsMixin)) {
      ...
      if (target instanceof EntityAvatar avatar) {
        ...
        if (avatar.getAvatar().getAvatarId() == 10000098
            || avatar.getAvatar().getAvatarId() == 10000096) target.setConvertToHpDebt(true);
      }
    }
  ...
  public AbilityInvokeEntry onAbilityInvoke(AbilityInvokeEntry invoke) throws Exception {
    invoke = normalizeMoonPhaseInvoke(invoke);
    Grasscutter.getLogger()
        .trace(
            "Ability invoke: "
                + invoke
    ...
    if (entity instanceof EntityAvatar avatarEntity
        && avatarEntity.getInstancedAbilities().isEmpty()) {
    ...
    switch (invoke.getArgumentType()) {
      ...
      case AbilityInvokeArgument_ABILITY_META_REINIT_OVERRIDEMAP -> this.handleReinitOverrideMap(
          invoke);
      ...
      case AbilityInvokeArgument_ABILITY_ACTION_GENERATE_ELEM_BALL -> this.handleGenerateElemBall(
          invoke);
      case AbilityInvokeArgument_ABILITY_META_GLOBAL_FLOAT_VALUE -> this.handleGlobalFloatValue(
          invoke);
      case AbilityInvokeArgument_ABILITY_META_CLEAR_GLOBAL_FLOAT_VALUE -> this
          .handleClearGlobalFloatValue(invoke);
      ...
      case AbilityInvokeArgument_ABILITY_META_ADD_SPECIAL_ENERGY_VALUE -> this
          .handleAddSpecialEnergy(invoke);
      ...
      default -> {
        ...
        if (typeVal > 100) {
          Grasscutter.getLogger()
              .trace(
                  "UnknownAbilityInvoke: type={} entityId={} dataSize={}",
                  typeVal,
                  invoke.getEntityId(),
                  invoke.getAbilityData().size());
        }
  ...
  private AbilityInvokeEntry normalizeMoonPhaseInvoke(AbilityInvokeEntry invoke)
      throws InvalidProtocolBufferException {
    var argumentType = invoke.getArgumentType();
    if (argumentType != AbilityInvokeArgument.AbilityInvokeArgument_ABILITY_META_GLOBAL_FLOAT_VALUE
        && argumentType
            != AbilityInvokeArgument.AbilityInvokeArgument_ABILITY_META_CLEAR_GLOBAL_FLOAT_VALUE) {
      return invoke;
    }

    var entry = AbilityScalarValueEntry.parseFrom(invoke.getAbilityData());
    String key = resolveAbilityString(entry.getKey());
    if (!MoonPhaseSystem.VERDANT_DEW_KEY.equals(key)) return invoke;

    var teamEntity = this.player.getTeamManager().getEntity();
    if (teamEntity == null) return invoke;

    int moonPhaseLevel = MoonPhaseSystem.getMoonPhaseLevel(this.player.getTeamManager());
    var normalized = invoke.toBuilder().setEntityId(teamEntity.getId());
    if (moonPhaseLevel > 0) {
      // A clear/consume from the client must not undo the compatibility fallback. Rewriting the
      // clear invoke also prevents the local client from immediately hiding the dew UI again.
      normalized
          .setArgumentType(
              AbilityInvokeArgument.AbilityInvokeArgument_ABILITY_META_GLOBAL_FLOAT_VALUE)
          .setAbilityData(
              entry
                  .toBuilder()
                  .setFloatValue(MoonPhaseSystem.MAX_VERDANT_DEW)
                  .build()
                  .toByteString());
    } else if (argumentType
        == AbilityInvokeArgument.AbilityInvokeArgument_ABILITY_META_GLOBAL_FLOAT_VALUE) {
      normalized.setAbilityData(entry.toBuilder().setFloatValue(0f).build().toByteString());
    }
    return normalized.build();
  }

  private static String resolveAbilityString(AbilityString value) {
    if (value == null) return null;
    if (value.hasStr() && !value.getStr().isEmpty()) return value.getStr();
    if (value.hasHash()) return GameData.getAbilityHashes().get(value.getHash());
    return null;
  }
  ...
  private void handleClearGlobalFloatValue(AbilityInvokeEntry invoke)
    ...
    if (MoonPhaseSystem.VERDANT_DEW_KEY.equals(key)) {
      var teamEntity = this.player.getTeamManager().getEntity();
      if (teamEntity == null) return;
      int moonPhaseLevel = MoonPhaseSystem.getMoonPhaseLevel(this.player.getTeamManager());
      float value = MoonPhaseSystem.synchronizeVerdantDew(teamEntity, moonPhaseLevel);
      teamEntity.onAbilityValueUpdate();
      this.player.sendPacket(
          new PacketServerGlobalValueChangeNotify(
              teamEntity, MoonPhaseSystem.VERDANT_DEW_KEY, value));
      return;
    }

    if (StellarConductorSystem.isStackKey(key)) {
      var teamEntity = this.player.getTeamManager().getEntity();
      if (teamEntity == null) return;
      StellarConductorSystem.setStacks(teamEntity, 0f);
      synchronizeStellarConductorValues();
      return;
    }
  ...
  private void handleAddSpecialEnergy(AbilityInvokeEntry invoke)
      throws InvalidProtocolBufferException {
    var head = invoke.getHead();
    AbilityMetaSpecialEnergyOuterClass.AbilityMetaSpecialEnergy abilityMetaSpecialEnergy =
        AbilityMetaSpecialEnergyOuterClass.AbilityMetaSpecialEnergy.parseFrom(
            invoke.getAbilityData());
    var entity = this.player.getScene().getEntityById(invoke.getEntityId());
    if (entity == null) {
  ...
  public void handleServerInvoke(AbilityInvokeEntry invoke) {
    ...
    Grasscutter.getLogger()
        .trace(
            "handleServerInvoke: action/mixin not found: localId={} ability={} knownActionIds={}"
                + " knownMixinIds={}",
            head.getLocalId(),
  ...
  private void handleOverrideParam(AbilityInvokeEntry invoke) throws Exception {
    ...
    if (instancedAbilityIndex < 0
        || instancedAbilityIndex >= entity.getInstancedAbilities().size()) {
        ...
        for (var variableChange : valueChanges.getOverrideMapList()) {
          ...
          for (var tv : talentVarList) {
            ...
            ProudSkillData proudSkill =
                GameData.getProudSkillDataMap().get(groupId * 100 + skillLevel);
          ...
          for (var mapEntry : talentVarMap.entrySet()) {
            ...
            for (var tv : mapEntry.getValue()) {
              if (computedVarOverrides.containsKey(tv.varName())) {
                varToAbilityNames.computeIfAbsent(tv.varName(), k -> new HashSet<>()).add(abilName);
              }
            }
          }
              ...
              avatarPatch.add(
                  AbilityInvokeEntry.newBuilder()
                      ...
                      .setArgumentType(
                          AbilityInvokeArgument.AbilityInvokeArgument_ABILITY_META_OVERRIDE_PARAM)
                      .setHead(
                          AbilityInvokeEntryHead.newBuilder().setInstancedAbilityId(i + 1).build())
                      .setAbilityData(
                          AbilityScalarValueEntry.newBuilder()
                              .setKey(
                                  AbilityString.newBuilder()
  ...
  private void handleModifierChange(AbilityInvokeEntry invoke) throws Exception {
    ...
    if (modChange.getAction() == ModifierAction.MODIFIER_ACTION_ADDED) {
      ...
      var parentAbilityName =
          resolvedParentName != null ? resolvedParentName : parentAbStr.getStr();

      if (instancedAbilityData == null) {
        Grasscutter.getLogger()
            .trace(
                "handleModifierChange: no ability data found for entityId={} parentAbility={}",
                invoke.getEntityId(),
                parentAbilityName);
        return;
      }
      ...
      if (instancedAbilityData.modifiers == null) {
        Grasscutter.getLogger()
            .trace(
                "handleModifierChange: modifiers map is null for ability={} entityId={}",
      ...
      if (modChange.getModifierLocalId() >= modifierArray.length) {
        Grasscutter.getLogger()
            .trace(
                "handleModifierChange: modifierLocalId={} out of bounds for ability={}"
                    + " (modifierCount={}), entityId={} modId={}",
                modChange.getModifierLocalId(),
                instancedAbilityData.abilityName,
                modifierArray.length,
                invoke.getEntityId(),
                head.getInstancedModifierId());
        return;
      }
  ...
  private void handleGlobalFloatValue(AbilityInvokeEntry invoke)
    ...
    if (MoonPhaseSystem.VERDANT_DEW_KEY.equals(key)) {
      ...
      if (teamEntity == null) return;
      int moonPhaseLevel = MoonPhaseSystem.getMoonPhaseLevel(this.player.getTeamManager());
      value = MoonPhaseSystem.synchronizeVerdantDew(teamEntity, moonPhaseLevel);
      teamEntity.onAbilityValueUpdate();
      this.player.sendPacket(
          new PacketServerGlobalValueChangeNotify(
              teamEntity, MoonPhaseSystem.VERDANT_DEW_KEY, value));
    ...
    if (StellarConductorSystem.isStackKey(key)) {
      var teamEntity = this.player.getTeamManager().getEntity();
      if (teamEntity == null) return;
      value = StellarConductorSystem.setStacks(teamEntity, value);
    ...
    if ("_ABILITY_Clorinde_Dodge_HealFlag".equals(key)
        && value == 0f
    ...
    if ("_Arlecchino_HPDebtsMark_Level".equals(key)) {
      ...
      if (value == 0f && prev != null && prev > 0f) {
        ...
        boolean isChargedAttack =
            arlecchinoChargedAttackTime > arlecchinoESkillTime
  ...
  private void applyClorindeBoL(EntityAvatar clorinde, float ratio) {
    ...
    scene.broadcastPacket(
        new PacketEntityFightPropUpdateNotify(clorinde, FightProperty.FIGHT_PROP_CUR_HP_DEBTS));
    scene.broadcastPacket(
        new PacketEntityFightPropChangeReasonNotify(
            ...
            ChangeHpDebtsReasonOuterClass.ChangeHpDebtsReason
                .CHANGE_HP_DEBTS_REASON_CHANGE_HP_DEBTS_ADD_ABILITY));
  }
  ...
  private float getArlecchinoBoLRatio(Avatar avatar, String varName) {
            ...
            skillId -> {
              ...
              var proudSkill =
                  GameData.getProudSkillDataMap()
                      .get(skillData.getProudSkillGroupId() * 100 + level);
    ...
    if (depot.getInherentProudSkillOpens() != null) {
      for (var inherent : depot.getInherentProudSkillOpens()) {
        ...
        var proudSkill =
            GameData.getProudSkillDataMap().get(inherent.getProudSkillGroupId() * 100 + 1);
  ...
  private void applyArlecchinoBoL(EntityAvatar arlecchino, int markLevel) {
    ...
    var scene = this.player.getScene();
    scene.broadcastPacket(
        new PacketEntityFightPropUpdateNotify(arlecchino, FightProperty.FIGHT_PROP_CUR_HP_DEBTS));
    scene.broadcastPacket(
        new PacketEntityFightPropChangeReasonNotify(
            arlecchino,
            FightProperty.FIGHT_PROP_CUR_HP_DEBTS,
  ...
  private void initializeEntityAbilities(EntityAvatar avatar) {
    ...
    var skillDepot =
        GameData.getAvatarSkillDepotDataMap().get(avatar.getAvatar().getSkillDepotId());
  ...
  private static Set<Integer> getMoonLightAbilityHashes() {
    ...
    if (s == null) {
      ...
      moonLightAbilityHashes = s = Set.copyOf(s);
    }
    return s;
  }

  private void synchronizeMoonPhaseTeamValues(boolean includeLevel) {
    int moonPhaseLevel = MoonPhaseSystem.getMoonPhaseLevel(this.player.getTeamManager());
    var teamEntity = this.player.getTeamManager().getEntity();
    if (teamEntity == null) return;
    ...
    teamEntity
        ...
        .put(MoonPhaseSystem.MOON_PHASE_LEVEL_KEY, (float) moonPhaseLevel);

    if (includeLevel) {
      this.player.sendPacket(
          new PacketServerGlobalValueChangeNotify(
              teamEntity, MoonPhaseSystem.MOON_PHASE_LEVEL_KEY, moonPhaseLevel));
    }
    this.player.sendPacket(
        new PacketServerGlobalValueChangeNotify(
            teamEntity, MoonPhaseSystem.VERDANT_DEW_KEY, verdantDew));
  }

  private void synchronizeStellarConductorValues() {
    var teamEntity = this.player.getTeamManager().getEntity();
    if (teamEntity == null) return;

    float stacks =
        StellarConductorSystem.setStacks(
            teamEntity, StellarConductorSystem.getStacks(teamEntity));
    for (String key : StellarConductorSystem.getStackKeys()) {
      this.player.sendPacket(new PacketServerGlobalValueChangeNotify(teamEntity, key, stacks));
    }

    for (var avatar : this.player.getTeamManager().getActiveTeam()) {
      if (avatar == null) continue;
      for (String key : StellarConductorSystem.getStackKeys()) {
        avatar.getGlobalAbilityValues().put(key, stacks);
        this.player.sendPacket(new PacketServerGlobalValueChangeNotify(avatar, key, stacks));
      }
  ...
  private void handleAddNewAbility(AbilityInvokeEntry invoke)
    ...
    if (entity == null) {
      Grasscutter.getLogger()
          .trace("handleAddNewAbility: entity not found: {}", invoke.getEntityId());
      return;
    }
    ...
    if (targetIndex < 0 || targetIndex >= InstancedAbilityList.MAX_ABILITIES) {
      Grasscutter.getLogger()
          .warn(
              "Rejected invalid instanced ability index {} for entity {}.",
              targetIndex,
              entity.getId());
      return;
    }
    ...
    boolean isTeamMoonPhase =
        "TeamAbility_MoonPhase".equals(abilityName)
            || abHash == Utils.abilityHash("TeamAbility_MoonPhase");
    boolean isTeamStellarConductor =
        StellarConductorSystem.TEAM_ABILITY.equals(abilityName)
            || abHash == Utils.abilityHash(StellarConductorSystem.TEAM_ABILITY);
    boolean isMoonLightAbility =
        (abilityName != null
                && abilityName.startsWith("Avatar_")
                && abilityName.endsWith("_MoonLight"))
            || getMoonLightAbilityHashes().contains(abHash);

    if (isTeamMoonPhase) {
      synchronizeMoonPhaseTeamValues(true);
    }

    if (isTeamStellarConductor) {
      synchronizeStellarConductorValues();
    }

    if (isMoonLightAbility) {
      synchronizeMoonPhaseTeamValues(false);
    }
    ...
    if (abilityData == null) {
      ...
      InstancedAbilityList.put(abilities, targetIndex, null);
      return;
    }

    var newAbility = new Ability(abilityData, entity, player);
    InstancedAbilityList.put(abilities, targetIndex, newAbility);
