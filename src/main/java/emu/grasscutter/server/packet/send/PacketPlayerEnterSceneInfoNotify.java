...
public class PacketPlayerEnterSceneInfoNotify extends BasePacket {
  ...
  public static Set<Integer> getMoonphaseIds() {
    return MoonPhaseSystem.getMoonPhaseAvatarIds();
  }
  ...
  public PacketPlayerEnterSceneInfoNotify(Player player) {
    ...
    AbilityScalarValueEntry scalarValue =
        AbilityScalarValueEntry.newBuilder()
            .setKey(
                AbilityStringOuterClass.AbilityString.newBuilder()
    ...
    long hexCount =
        player.getTeamManager().getActiveTeam().stream()
            ...
            .filter(java.util.Objects::nonNull)
            .filter(avatar -> getHexenzirkelIds().contains(avatar.getAvatarId()))
            .count();

    AbilityScalarValueEntry hexLevel =
        AbilityScalarValueEntry.newBuilder()
            .setKey(
    ...
    AbilityScalarValueEntry moonPhaseLevelEntry =
        AbilityScalarValueEntry.newBuilder()
            .setKey(
                AbilityString.newBuilder()
                    ...
                    .build())
            .setFloatValue(moonPhaseLevel)
            .build();

    AbilitySyncStateInfo.Builder teamInfo =
        AbilitySyncStateInfo.newBuilder()
            .addSgvDynamicValueMap(scalarValue)
            .addSgvDynamicValueMap(hexLevel)
            .addSgvDynamicValueMap(moonPhaseLevelEntry)
            // Some 7.0 clients restore team values only from the regular map on reconnect.
            .addDynamicValueMap(moonPhaseLevelEntry);

    if (moonPhaseLevel > 0) {
      var dewEntry = buildScalarValue(MoonPhaseSystem.VERDANT_DEW_KEY, verdantDew);
      teamInfo.addDynamicValueMap(dewEntry).addSgvDynamicValueMap(dewEntry);
    }

    for (String key : StellarConductorSystem.getStackKeys()) {
      teamInfo.addDynamicValueMap(buildScalarValue(key, stellarStacks));
    }
    ...
    PlayerEnterSceneInfoNotify.Builder proto =
        PlayerEnterSceneInfoNotify.newBuilder()
            .setCurAvatarEntityId(currentAvatar != null ? currentAvatar.getId() : 0)
    ...
    for (EntityAvatar avatarEntity : player.getTeamManager().getActiveTeam()) {
      if (avatarEntity == null || avatarEntity.getAvatar() == null) continue;
      ...
      for (String key : StellarConductorSystem.getStackKeys()) {
        avatarEntity.getGlobalAbilityValues().put(key, stellarStacks);
        avatarAbilityInfo.addDynamicValueMap(buildScalarValue(key, stellarStacks));
      }
  ...
  public static void buildAvatarAbilityVars(
      EntityAvatar avatarEntity, AbilitySyncStateInfo.Builder info) {
    ...
    AvatarSkillDepotData depotData =
  ...
  private static AbilityScalarValueEntry buildScalarValue(String key, float value) {
